...
class Ingredient(models.Model):
    ...
    is_natural = models.BooleanField(
        default=False, help_text="TRUE if the ingredient does not contain added color, artificial flavors, or synthetic substances."
    )
    ...
    TYPE_FLAVOR = 3
    TYPE_YEAST = 4
    INGREDIENT_TYPES = ((TYPE_SUGAR, "Sugar"), (TYPE_SOLVENT, "Solvent"), (TYPE_FLAVOR, "Flavor"), (TYPE_YEAST, "Yeast"))
    type = models.IntegerField(choices=INGREDIENT_TYPES, default=TYPE_SUGAR)
    # Ingredient subtypes.
    SUGAR_HONEY = 101
    SUGAR_MALT = 102
    SUGAR_OTHER = 103
    SUGAR_TYPES = ((SUGAR_HONEY, "Honey"), (SUGAR_MALT, "Malt"), (SUGAR_OTHER, "Other"))
    SOLVENT_WATER = 201
    SOLVENT_GRAPE = 202
    SOLVENT_APPLE = 203
    SOLVENT_FRUIT = 204
    ...
    FLAVOR_FRUIT = 304
    FLAVOR_OTHER = 305
    FLAVOR_TYPES = ((FLAVOR_SPICE, "Spice"), (FLAVOR_GRAPE, "Grape"), (FLAVOR_APPLE, "Apple"), (FLAVOR_FRUIT, "Fruit"), (FLAVOR_OTHER, "Other"))
    YEAST_DRY = 401
    YEAST_WET = 402
    YEAST_TYPES = ((YEAST_DRY, "Dry"), (YEAST_WET, "Wet"))
    INGREDIENT_SUBTYPES = ((TYPE_SUGAR, SUGAR_TYPES), (TYPE_SOLVENT, SOLVENT_TYPES), (TYPE_FLAVOR, FLAVOR_TYPES), (TYPE_YEAST, YEAST_TYPES))
    ...
    INGREDIENT_STATES = ((STATE_SOLID, "Solid"), (STATE_LIQUID, "Liquid"), (STATE_OTHER, "Other"))
    # Certain states are only valid for certain types.
    STATE_TYPES = ((STATE_SOLID, (TYPE_SUGAR,)), (STATE_LIQUID, (TYPE_SOLVENT,)), (STATE_OTHER, (TYPE_FLAVOR, TYPE_YEAST)))

    state = models.IntegerField(choices=INGREDIENT_STATES, default=STATE_SOLID)
    sg = models.DecimalField(
        "Specific Gravity", max_digits=4, decimal_places=3, default=Decimal("1.000"), help_text="Specific gravity (water is 1.000, honey is usually 1.422)"
    )
    sh = models.DecimalField(
        "Specific Heat", max_digits=3, decimal_places=2, default=Decimal("1.00"), help_text="Specific heat (water is 1.00, honey is usually 0.57)"
    )
    ...
    cpu = models.DecimalField(
        ...
        help_text="Cost in USD per unit (kilogram if solid, liter if liquid, other if other)",
    )
...
class Parent(models.Model):
    ...
    CATEGORY_OTHER = 260
    CATEGORY_ALL = 290
    MEAD_CATEGORIES = ((CATEGORY_TRADITIONAL, "Traditional Mead"), (CATEGORY_MELOMEL, "Melomel"), (CATEGORY_OTHER, "Other Meads"), (CATEGORY_ALL, "All Meads"))
    TRADITIONAL_DRY = 241
    TRADITIONAL_SEMI_SWEET = 242
    TRADITIONAL_SWEET = 243
    TRADITIONAL_CATEGORIES = ((TRADITIONAL_DRY, "Dry Mead"), (TRADITIONAL_SEMI_SWEET, "Semi-Sweet Mead"), (TRADITIONAL_SWEET, "Sweet Mead"))
    MELOMEL_CYSER = 251
    MELOMEL_PYMENT = 252
    MELOMEL_OTHER = 253
    MELOMEL_CATEGORIES = ((MELOMEL_CYSER, "Cyser"), (MELOMEL_PYMENT, "Pyment"), (MELOMEL_OTHER, "Other Fruit Melomel"))
    OTHER_METHEGLIN = 261
    OTHER_BRAGGOT = 262
    OTHER_OPEN_CATEGORY = 263
    OTHER_CATEGORIES = ((OTHER_METHEGLIN, "Metheglin"), (OTHER_BRAGGOT, "Braggot"), (OTHER_OPEN_CATEGORY, "Open Category Mead"))
    ALL = 291
    ALL_CATEGORIES = ((ALL, "All"),)

    MEAD_SUBCATEGORIES = (
        (CATEGORY_TRADITIONAL, TRADITIONAL_CATEGORIES),
    ...
    MEAD_DESCRIPTIONS = {
        TRADITIONAL_DRY: (
            "A traditional mead with subtle honey aroma but no significant aromatics. Minimal residual "
            + "sweetness with a dry finish, and a light to medium body. Similar to a dry white wine. "
            + "(Based on BJCP Style Guidelines 2008)"
        ),
        TRADITIONAL_SEMI_SWEET: (
            "A traditional mead with noticeable honey aroma and subtle to moderate residual sweetness "
            + "with a medium-dry finish.  Body is medium-light to medium-full.  Similar to a semi-sweet "
            + "(or medium-dry) white wine.  (thanks to BJCP Style Guidelines 2008)"
        ),
        TRADITIONAL_SWEET: (
            "A traditional mead with dominating honey aroma, often moderately to strongly sweet.  The body "
            + "is generally medium-full to full, and may seem like a dessert wine.  (thanks to BJCP Style Guidelines 2008)"
        ),
        ...
        OTHER_OPEN_CATEGORY: (
            "A honey-based beverage that either combines ingredients from two or more of the other mead "
            + "sub-categories, is a historical or indigenous mead (e.g., tej, Polish meads), or is a mead "
            + "that does not fit into any other category.  Any specialty or experimental mead using additional "
            + "sources of fermentables (e.g., maple syrup, molasses, brown sugar, or agave nectar), additional "
            + "ingredients (e.g., vegetables, liquors, smoke, etc.), alternative processes (e.g., icing, oak-aging) "
            + "or other unusual ingredient, process, or technique would also be appropriate in this category."
        ),
...
class Product(SIPParent):
    slug = models.SlugField(max_length=255, blank=True, unique=True, help_text="Unique value for product page URL, created from brewname and batchletter.")
...
class ProductReview(models.Model):
    RATINGS = ((5, "5 - Outstanding"), (4, "4 - Excellent"), (3, "3 - Very Good"), (2, "2 - Good"), (1, "1 - Fair"))
