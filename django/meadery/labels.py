    ...
    def __init__(self, buffer=None, debug=False):
        ...
        self.framelist = [
            (
                ...
                self.height - self.bottom_margin - self.labelh - x * (self.row_gutter + self.labelh),
                self.labelw,
                self.labelh,
            )
        ...
        self.doc.addPageTemplates(
            [
                PageTemplate(
                    frames=[
                        Frame(a, b, c, d, leftPadding=0, bottomPadding=0, rightPadding=0, topPadding=0, showBoundary=(1 if self.debug else 0))
...
class Label:
    ...
    def render(self, sheet):
        ...
        tabledata = [[SITE_NAME, ""], [self.batch.title, ""], [self.batch.brewname, self.seq]]
        tablestyle = [
            ("SPAN", (0, 0), (1, 0)),
