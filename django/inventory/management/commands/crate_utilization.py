...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--warehouse", help="Warehouse for which table is generated.")
        parser.add_argument("--full", action="store_true", dest="full", default=False, help="Include full crates in table")
        parser.add_argument("--empty", action="store_true", dest="empty", default=False, help="Include empty crates in table")
    ...
    def handle(self, *args, **options):
        ...
        for crate in sorted(
            Crate.objects.filter(bin__in=Bin.objects.filter(shelf__in=Shelf.objects.filter(row__in=Row.objects.filter(warehouse=warehouse)))),
            key=lambda c: c.jars * 1.0 / c.capacity,
        ):
            if (
                (crate.jars != crate.capacity and crate.jars != 0)
                or (options["full"] and crate.jars == crate.capacity)
                or (options["empty"] and crate.jars == 0)
            ):
                # The warehouse is superfluous here
                loc = crate.bin.longname[len(warehouse.longname) + 1 :]  # noqa: E203
