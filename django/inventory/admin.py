...
class JarAdmin(SmarterModelAdmin):
    ...
    class MoveMultipleJarsForm(forms.Form):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fields["dest"].queryset = (
                Crate.objects.annotate(room=F("capacity") - Sum(Case(When(jar__is_active=True, then=1), default=0), output_field=IntegerField()))
                .filter(room__gte=count)
                .order_by("number")
            )
    ...
    def move_multiple_jars(self, request, queryset):
        ...
        return render(request, "admin/move_multiple_jars.html", {"jars": queryset, "move_multiple_jars_form": form})
